...
const Form2 = ({
  ...
  const handleInputFocus = async (e) => {
    ...
    const ul = labelInput.parentNode.parentNode.getElementsByClassName(
      'fsOptionsList'
    )[0];
    ul.style.display = '';
  };
  ...
  const handleInputOnBlur = async (e) => {
    ...
    const valueInput = labelInput.parentNode.getElementsByClassName(
      'fsValueInput'
    )[0];
  ...
  const doSearch = (e, reset = false) => {
    ...
    const whereSearch = labelInput.parentNode.parentNode.getElementsByClassName(
      'fsOption'
    );
    const whatSearch = reset
      ? ''
      : removeAccents(labelInput.value.toLowerCase());
  ...
  const handleLiOnMouseDown = (e) => {
    ...
    const labelInput = e.target.parentNode.parentNode.parentNode.getElementsByClassName(
      'fsLabelInput'
    )[0];
    const valueInput = e.target.parentNode.parentNode.parentNode.getElementsByClassName(
      'fsValueInput'
    )[0];
  ...
  const handleExtendIconMouseDown = (e) => {
    const labelInput = e.target.parentNode.getElementsByClassName(
      'fsLabelInput'
    )[0];
    const ulVisible =
      e.target.parentNode.parentNode.getElementsByClassName('fsOptionsList')[0]
        .style.display !== 'none';
    if (!ulVisible) {
  ...
  const options = data.map((e, i) => {
    ...
    return (
      ...
      >
        {e[label]}
        <span className="fsOptionValue" hidden>
          {e[value]}
        </span>
        <p className={`${styles.fsOptionExtraInfo} fsOptionExtraInfo`}>
          {e[extraInfo]}
        </p>
      </li>
    );
  ...
  return (
    <div className={`${name}Div fsContainer ${styles.fsContainer}`}>
      <Grid item xs={12} sm={8}>
        <div className={`fsInputContainer ${styles.fsInputContainer}`}>
          <p></p>
          <input
            className={`${name}LabelInput fsLabelInput ${styles.fsLabelInput} ${
              className ? className : ''
            }`}
